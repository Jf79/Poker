...
class RoundSpec extends AnyWordSpec with Matchers {
    ...
    var round = new Round(player, deck, bet)
    ...
    "A round when it set the player cards " should {
        "have a player with a hand of 5 cards" in {
    ...
    val hand = Array(new Card(HEART, TWO, 2), new Card(HEART, THREE, 3), new Card(HEART, FOUR, 4), 
    new Card(HEART, FIVE, 5), new Card(HEART, SIX, 6))
    val cards = Array(new Card(SPADE, TWO, 2), new Card(SPADE, THREE, 3), new Card(SPADE, FOUR, 4), 
    new Card(SPADE, FIVE, 5), new Card(SPADE, SIX, 6))
